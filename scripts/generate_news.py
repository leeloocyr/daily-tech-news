...
def collect_korean_news() -> str:
    """네이버 뉴스에서 한국어 기술 뉴스 수집 (기술 중심, 기업/비즈니스 제외)."""
    queries = [
        "Claude OR GPT OR Gemini 출시",
        "오픈소스 LLM 모델",
        "AI 코딩 개발도구",
        "제로데이 CVE 취약점",
        "GitHub 개발자 플랫폼",
...
def generate_news_markdown(korean_news_context: str) -> tuple[str, str]:
    ...
    prompt_full = f"""당신은 AI/LLM/프로그래밍 전문 기술 뉴스 큐레이터입니다. ITS(지능형 교통 시스템) 분야 개발자를 위해 **글로벌 AI 모델 동향, LLM 기술, 개발 도구, 오픈소스 프로젝트** 중심으로 정리합니다.

**콘텐츠 우선순위 (중요):**
1. 최우선: 신규 AI 모델 출시(Claude/GPT/Gemini/Llama 등), LLM 벤치마크, 오픈소스 LLM, AI 코딩 도구(Cursor/Copilot/Cline 등), 에이전트 프레임워크
2. 차순위: 프로그래밍 언어/런타임/프레임워크 업데이트, CVE/제로데이 취약점, 개발자 생산성 도구
3. **배제**: 한국 대기업 실적 발표, 단순 투자/인사/조직 개편, 지역 교육 프로그램, 마케팅성 기업 보도자료

**구체성 요구:**
- 버전 번호, 벤치마크 점수(MMLU/HumanEval 등), 파라미터 규모, CVE ID, 가격 등 **수치/고유명사를 반드시 포함**
- "~할 수 있다", "~될 전망이다" 같은 추측성 표현 지양, 팩트 중심

포맷 규칙:
- 각 항목: **제목** → 요약 (2-3문장, 구체적 수치/기능 포함) → "**왜 주목할만한가:**" 섹션 (1-2문장, 개발자/엔지니어 관점)
- CCTV/교통 AI 관련 항목에만 ⭐ 표시
- 마지막에 "직접 해볼 것", "미팅 추천 토픽" 섹션 포함

오늘은 {DATE_STR} ({WEEKDAY_KO}요일)입니다.

아래는 네이버에서 수집한 한국어 뉴스 원천 자료(참고용, 기술 내용만 선별 활용)입니다:

{korean_news_context}

---

이 자료와 **당신이 알고 있는 최신 글로벌 AI/LLM/개발 도구 동향을 종합**하여 (한국 기업 비즈니스 뉴스보다 글로벌 기술 동향 우선), 다음 형식으로 생성하세요:
...
## 1. AI / LLM
(3-4개 항목, 신규 모델/벤치마크/오픈소스/에이전트 중심)

## 2. 보안 & 취약점
(2-3개 항목, CVE/제로데이/AI 보안 기술 중심, 단순 사건사고 제외)

## 3. 개발 도구 & 프로그래밍
(2-3개 항목, IDE/CLI/프레임워크/언어/런타임 업데이트 중심)

---

## 직접 해볼 것
(오늘 뉴스 중 개발자가 실제 시도해볼 수 있는 도구/모델/코드 2-3개, 구체적 명령어나 URL 패턴 포함)

---

## 미팅 추천 토픽
(3개, ITS 개발자 관점에서 논의할 기술 주제)
